...
const update = async (req, res) => {
    const id = await Requirement.findById(req.params.id).select('Candidate');
    for (let i = 0; i < id.Candidate.length; i++) {
        if (req.body.Candidate.includes(id.Candidate[i].toString()) === false) {
            req.body.Candidate.push(id.Candidate[i].toString());
        }
